...
let bossMuertoT = -1;
let cartaHintT = 0;
let cartaAutoT = -1;             /* cuenta atrás: la carta se abre sola 5 s tras tocar suelo */
...
addEventListener('keydown', e => {
  if(state === 'carta') return;   /* la carta bloquea todo el input del juego */
  K[e.code] = true;
...
function actualizarCartaDrop(dt){
  ...
  if(cartaMesh.position.y > yBase){
    ...
    if(cartaMesh.position.y <= yBase && !cartaTocoSuelo){
      cartaTocoSuelo = true;
      cartaAutoT = 5;                /* la carta se abrirá sola a los 5 segundos */
    ...
    /* AUTO-APERTURA: 5 segundos tras tocar el suelo, sin importar si te acercas */
    if(!cartaAbierta){
      cartaAutoT -= dt;
      if(cartaAutoT <= 0) abrirCarta();
    }
  }
}
function abrirCarta(){
...
function mostrarCartaHTML(){
  ...
  UI.hud.classList.add('oculto');          /* el juego pasa a segundo plano total */
  ...
  /* la rueda SOLO hace scroll manual del texto, jamás zoom de cámara */
  const t = UI.cartaTxto;
